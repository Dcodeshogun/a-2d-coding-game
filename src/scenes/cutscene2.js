export default class TerminalCutscene2 extends Phaser.Scene {
  constructor() {
    super({ key: 'TerminalCutscene2' });
  }
  ...
  {
    speaker: '6O',
    message: "Nice work out there, 2B! The factory’s signal is stable again — not too much collateral damage this time, I hope?",
    choices: [
      { text: "Area’s secure. Machines neutralized.", response: "report" },
      { text: "Collateral was... unavoidable.", response: "dry" }
    ]
  },
  {
    speaker: '6O',
    message: "Well, command’s impressed. But there’s another node cluster acting up — this one’s buried in the old mall ruins.",
    choices: [
      { text: "Understood. Sending coordinates to Pod.", response: "professional" },
      { text: "Guess I won’t be getting a break, huh?", response: "tired" }
    ]
  },
  {
    speaker: '6O',
    message: "You know how it is — the code never sleeps. Uploading mission data now.",
    choices: [
      { text: "Acknowledged. Moving out.", response: "proceed" },
      { text: "I’ll patch it faster than last time.", response: "confident" }
    ]
  }
];
  ...
  createMissionInfo(panelWidth, startY) {
    ...
    const missionData = [
      'OBJECTIVE: Shopping Mall Node Cluster Neutralization',
      'PRIORITY: HIGH',
      'OPERATOR: 6O',
...
createSystemDiagnostics(panelWidth, startY) {
  ...
  showNotification() {
    ...
    const icon = this.add.image(-notificationWidth + 20, 17, 'icon_6o').setOrigin(0.5).setScale(0.35);
  ...
  show2BResponse(text) {
    ...
    const icon = this.add.image(-notificationWidth + 20, 17, 'icon_2b').setOrigin(0.5).setScale(0.35);
    ...
    this.time.delayedCall(300, () => {
      this.cameras.main.fade(500, 0, 0, 0);
      this.time.delayedCall(500, () => {
        this.scene.start('QuizScene', { level: 2, nextLevel: 2,skipInstruction: true }); // Change to your game scene key
      });
    });
  }
}
